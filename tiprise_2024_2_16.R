...
# nl.vec
# #
# #test sum:  # should be equal to n.vec
# nh.vec+nl.vec
# 
# # social norm num of tipped services
# (s0 = 1)
# (sig = 0.6)# sigma in paper: min frac of high tippers to trigger change in social norm (adding 1 more service)
# s.vec = as.numeric(rep(NA, length(t.vec)))
# sh.vec = as.numeric(rep(NA, length(t.vec)))
# sl.vec = as.numeric(rep(NA, length(t.vec)))
# #
# (sh.vec[1] = s0+1)# eq (1) in paper
# (sl.vec[1] = s0)
# (s.vec[1] = ifelse(nh.vec[1]/n.vec[1] > sig, s0+1, s0))
# #
# for (t in 2:length(t.vec)) {
#   sh.vec[t] = s.vec[t-1]+1
#   sl.vec[t] = s.vec[t-1]
#   (s.vec[t] = ifelse(nh.vec[t]/n.vec[t] > sig, s.vec[t-1]+1, s.vec[t-1]))
#   }
# data.frame(t.vec, s.vec, sh.vec, sl.vec, nh.vec/n.vec)
# 
